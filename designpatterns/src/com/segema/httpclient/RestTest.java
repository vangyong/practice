...
public class RestTest {
	...
	public static ResultVo test(String userName, String paaWord) throws Exception {
		...
		JSONObject userJsonObj = JSONObject.fromObject(parameter); 
		//下面三行不是必须参数
		...
		post.setRequestBody(userJsonObj.toString());
		...
		try {
			...
			if(status==200){
				...
				if(returnStr!=null&&(!"".equals(returnStr))){
					JSONObject returnObj = new JSONObject().fromObject(returnStr);
					resultVo =(ResultVo)JSONObject.toBean(returnObj, ResultVo.class); 
				}else{
