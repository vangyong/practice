...
public class BulkProcessDemo {
    ...
    public static void createIndex(String indexName) throws IOException {
        String[] ips = {"192.168.101.106:9200"};
        ...
        requestIndex.settings(Settings.builder().put("index.number_of_shards", 5).put("index.number_of_replicas", 0)
    ...
    /**
     * @description 将mysql数据查出组装成es需要的map格式，通过批量写入es
     * @param tableName
     */
    private static void writeMysqlDataToES(String tableName) {

        RestHighLevelClient client =
            new RestHighLevelClient(RestClient.builder(new HttpHost("192.168.101.106", 9200, "http")));
        ...
        try {
            ...
            for (HashMap<String, String> hashMap2 : dataList) {
                bulkProcessor
                    .add(new IndexRequest(tableName.toLowerCase(), "gzdc", hashMap2.get("S_GUID")).source(hashMap2));
            }
